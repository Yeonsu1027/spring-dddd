document.addEventListener("DOMContentLoaded", () => {
  ...
  cust_body?.addEventListener("click", cust_body_onClick_handler); //함수 괄호금지

  btn_update?.addEventListener("click", (e) => {
    const ccode = e.target.dataset.ccode;
    // alert(ccode);
    document.location.href = `${rootPath}/customer/update?ccode=${ccode}`; // input box 이름이랑 달라야함 update?ccode
  });
