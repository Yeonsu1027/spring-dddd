...
public class SecurityContextConfig extends WebSecurityConfigurerAdapter{
	...
	public void configure(WebSecurity web) throws Exception {
		...
		web.ignoring().antMatchers("/staitc/**"); // 이폴더는 시큐리티에서 관여하지말아라
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/hello").hasAnyRole("USER","ADMIN") // .antMatchers: 폴더패턴 지정함수
		.antMatchers("/user/mypage").hasAnyRole("USER","ADMIN") // 로그인이 되어있고, 권한이 USER, ADMIN
		.antMatchers("/user/mypage").authenticated() // 로그인이 되어잇는 경우
		.antMatchers("/**").permitAll() // 그외 모든
		.anyRequest().permitAll() // 그외 모든 요청은 그냥 통과
		.and().formLogin(); // and : http다시시작
