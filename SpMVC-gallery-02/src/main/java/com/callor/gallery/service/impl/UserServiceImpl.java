...
public class UserServiceImpl implements UserService{
	...
	private final RoleDao roleDao; //의존관계역전 spring 한테달라고하는것
	 ...
	 * 현재 method 는 정상적으로 수행이 된 것으로 인식되어 버린다
	...
	public int create_user(UserVO userVO) throws DataAccessException{ // service 인터페이스에도적어야함
		...
		if(resultVO == null || resultVO.size() < 1) { //데이터없으면
			grantList.add(
	...
	public void create_table() {
		...
		try {
			userDao.create_table(null);	// 생성순서 주의 FK
		} catch (Exception e) { // 오류 나면 아무것도 안하고 통과
