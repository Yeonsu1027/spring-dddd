...
public class FileServiceImplV1 implements FileService{
	 ...
	 * 사용하는 변수가 있을경우 이 변수는 protected 로 선언해야 한다
	 * 
	 * 또한 유의해야 할 것은
	 * 생성자는 상속 되지 않는다.
	 * 
	...
	@Override
	public String fileUp(MultipartFile file) throws Exception { // 밑줄그어지면 throws 클릭하고 Exception제외 지우기 다시클릭하면 interface에도 throws
		String fileName = file.getOriginalFilename();
		if(fileName.isBlank()) {
			return null;
		}
		File path = new File(upLoadPath);// java.io import
		// 업로드할 폴더가 없으면 폴더 생성하기 
		if(!path.exists() ) {
			/*
			 * path.mkdir(), path.mkdirs()
			 * 폴더를 생성하는 method
			 * mkdir() : 한 개의 폴더만 생성하기 : app/upload 폴더를 생성할 경우
			 * 		app 폴더가 없으면 오류가 발생
		...
		// 원래의 파일이름 앞에 uuid 를 부착하여 업로드할 파일 이름 생성 
		...
		// DB에 파일이름을 저장하기 위해서변경된 파일이름을 return 하기
		return fileName;
	}
	...
	public List<ImageVO> filesUp(MultipartHttpServletRequest files) {
		...
		/*
		 * view의 form input[type='file'] tag의 name(=files) 속성을 통하여
		 * 파일 리스트 추출하기
		 * */
		List<MultipartFile> fileList = files.getFiles("files"); //input.jsp 의 file 태그 이름
