...
/*
 * 프로젝트에서 한 개의 interface 를 implements 하여 다수의 클래스를 정의 하는 경우가 있다
 * 이럴때 사용하는 곳에서 interface 를 사용하여 객체를 선언하고
 * 생성자나, setter 등을 통하여 객체(bean)를 주입받는다
 * 이럴때 한 개의 interface 를 implements 한 객체가 2개 이상일 경우
 * Spring DI 는 어떤 클래스를 주입해야 할지 알 수 없다 
 * Spring DI 에게 어떤 클래스(객체)를 주입해야 할지 알려주기 위하여
 * Service Annotaion 에 이름을 부여하고,
 * 주입받는 곳에서는 @Qualifier 를 사용하여 명시적으로 어떤 클래스(객체)를 주입받을지
 * 선언해주어야 한다. 
 * */
@Service(QualifierConfig.SERVICE.FILE_SERVICE_V1) 			// 문자열 "fileServiceImplV2" 매직스트링방지
