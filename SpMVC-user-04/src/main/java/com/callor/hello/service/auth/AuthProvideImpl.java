...
public class AuthProvideImpl implements org.springframework.security.authentication.AuthenticationProvider{
	
	/*
	 *  @Autowired
	 *  기본생성자를 통하여 UserDao type 의 userDao 객체를 주입받아라 라는 선언
	 *  Autowired 를 사용하여 생성자 주입을 하게 되면
	 *  		 userDao 객체에 final을 사용할 수 없다
	 *  클래스 영역, 필드 영역에 선언되는 객체(클래스를 이용하여 선언한 변수들)는
	...
	public AuthProvideImpl(@Qualifier("passEncoderV1") // 유틸리티랑 다른이름을쓸때
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		if(userVO == null) {
			 ...
			 * */
			 throw new UsernameNotFoundException(
					 String.format("USERNAME 확인 : %s 는 없음", username)
					 );
		}
		...
		List<RoleVO> roles = roleDao.findByUserName(username); // 유저의role정보가져오기
